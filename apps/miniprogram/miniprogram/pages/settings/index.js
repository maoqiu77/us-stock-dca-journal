...
Page({
  ...
  restoreBackup() {
    ...
    wx.showModal({ title: '用备份替换当前完整工作区？', content: `将恢复 ${this.data.preview.openings} 条期初持仓、${this.data.preview.trades} 笔交易、${this.data.preview.personalNotes} 条个人记录、${this.data.preview.runs} 份分析。${this.data.preview.version === 7 ? '这是 v7 完整备份。' : this.data.preview.complete ? '这是旧版完整备份，将迁移到 v7。' : '这是旧备份，将创建新工作区。'}`, confirmText: '确认恢复', success: result => {
