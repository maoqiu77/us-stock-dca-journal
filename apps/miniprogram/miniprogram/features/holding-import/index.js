...
Page({
  data: { error: '', capability: null, images: [], progress: '', failedImages: [], imagePath: '', imageSize: 0, mimeType: 'image/png', stage: 'select', rows: [], busy: false, expectedRevision: 0, previewData: null, selectedCount: 0 },
  async onLoad() {
  ...
  chooseImage() {
    ...
    wx.chooseMedia({ count: 9, mediaType: ['image'], sizeType: ['compressed'], sourceType: ['album', 'camera'], success: result => {
      ...
      this.setData({ images, imagePath: images[0].path, rows: [], selectedCount: 0, stage: 'select', failedImages: [], progress: '', previewData: null, error: '' });
    } });
  },
    ...
    try {
      for (const file of this.data.images) {
        ...
        try {
          ...
          drafts.push({ number: file.number, rows: draft.rows });
      ...
      const resolved = await this.resolveRows(drafts.slice().sort((a, b) => a.number - b.number).flatMap(draft => draft.rows.map((row, index) => ({ ...row, sourceImage: draft.number, sourceRow: index + 1 }))));
      const previous = new Map(this.data.rows.map(row => [row.rowId, row]));
      const rows = resolved.map(row => previous.get(row.rowId) || row);
      this._token = '';
      this.setData({ rows, previewData: null, failedImages, stage: rows.length ? 'review' : 'select', selectedCount: rows.filter(item => item.selected).length, progress: `已识别 ${drafts.length}/${this.data.images.length} 张`, error: failedImages.length ? `${failedImages.length} 张识别失败，可重试失败图片或先导入成功的结果。` : '' });
  ...
  async resolveRows(input) {
    ...
    for (let index = 0; index < input.length; index++) {
      ...
      const instrument = match ? { symbol: match.symbol, name: match.name, market: match.market, currency: match.currency, assetType: match.asset_type, status: 'verified', instrumentKey: match.instrument_key } : { symbol: code, name, market: raw.currency === 'CNY' ? 'CN' : 'US', currency: raw.currency || 'USD', assetType: raw.currency === 'CNY' && /^\d{6}$/.test(code) && /基金|混合|QDII|联接|债券|股票.*[A-C]$/i.test(name) && !/ETF/i.test(name) ? 'FUND' : /ETF/i.test(name) ? 'ETF' : 'STOCK', status: 'unverified', instrumentKey: '' };
  ...
  toggleEdit(event) { const rows = this.data.rows.slice(), index = Number(event.currentTarget.dataset.index); if (!rows[index]) return; rows[index] = { ...rows[index], editing: !rows[index].editing }; this.setData({ rows }); },
  ...
  pickCandidate(event) { if (this.data.busy) return; const index = Number(event.currentTarget.dataset.index), candidateIndex = Number(event.currentTarget.dataset.candidate), rows = this.data.rows.slice(), row = rows[index], candidate = row?.candidates?.[candidateIndex]; if (!row || !candidate) return; rows[index] = { ...row, symbol: candidate.symbol, name: candidate.name, market: candidate.market, currency: candidate.currency, assetType: candidate.asset_type, status: 'verified', instrumentKey: candidate.instrument_key, issues: row.issues.filter(item => !item.includes('标的')) }; rows[index].existing = allPositions().find(item => item.symbol === candidate.symbol && item.market === candidate.market && item.currency === candidate.currency) || null; rows[index].replaceApproved = false; rows[index].costRemovalApproved = false; rows[index].selected = rows[index].issues.length === 0 && !rows[index].existing; this.updateRows(rows); },
  setAssetType(event) { if (this.data.busy) return; const index = Number(event.currentTarget.dataset.index), assetType = event.currentTarget.dataset.type, rows = this.data.rows.slice(); if (!rows[index] || !['STOCK', 'ETF', 'FUND'].includes(assetType)) return; rows[index] = { ...rows[index], assetType }; this.updateRows(rows); },
  toggleRow(event) { if (this.data.busy) return; const index = Number(event.currentTarget.dataset.index), rows = this.data.rows.slice(); if (!rows[index]) return; rows[index] = { ...rows[index], selected: !rows[index].selected }; this.updateRows(rows); },
  ...
  confirmManual(event) { if (this.data.busy) return; const index = Number(event.currentTarget.dataset.index), rows = this.data.rows.slice(), row = rows[index]; if (!row || !row.symbol.trim() || !row.name.trim() || !row.market.trim() || !/^[A-Za-z]{3}$/.test(row.currency.trim())) { this.setData({ error: '请完整填写名称、代码、市场和三位币种。' }); return; } rows[index] = { ...row, status: 'unverified', issues: row.issues.filter(item => !item.includes('标的')), replaceApproved: !!row.existing, selected: row.issues.filter(item => !item.includes('标的')).length === 0 }; this.updateRows(rows); },
  ...
  async preview() { if (this.data.busy) return; try { const selected = this.data.rows.filter(item => item.selected); if (!selected.length) throw Error('请至少选择一项有效持仓。'); if (selected.length > 20) throw Error('每批最多导入 20 项，请取消部分选择后分批导入。'); if (selected.some(item => item.issues.length || item.existing && !item.replaceApproved)) throw Error('请先解决已选行的标的、数量、成本或替换确认问题。'); const result = service.previewHoldingImport(this.request()); this._token = result.contentToken; this.setData({ previewData: result, error: '', busy: true });
      ...
      const confirmed = await new Promise(resolve => wx.showModal({ title: `导入 ${selected.length} 项持仓？`, content: content + '\n未匹配行情的项目仍可保存；已有持仓会更新为上述数量，不会新增买卖记录。', confirmText: '确认导入', success: value => resolve(!!value.confirm), fail: () => resolve(false) }));
