...
const workspaceFields = { instance_id: idSchema, portfolio_id: idSchema, root_generation: z.number().int().nonnegative(), migration: migrationStateSchema, journal: z.array(journalEntrySchema).max(10000), conversations: z.array(conversationSchema).max(1000), messages: z.array(messageSchema).max(20000), runs: z.array(analysisRunSchema).max(5000), sources: z.array(sourceSnapshotSchema).max(10000), policies: z.array(policySchema).max(1000) };
