import { z } from 'zod';
import { researchTurnEnvelopeV1Schema, researchTurnEnvelopeV2Schema } from '@portfolio/ai-context';
...
export const outboxStatusSchema = z.enum(['prepared', 'submitting', 'running', 'outcome_unknown', 'remote_succeeded', 'local_save_pending', 'ack_pending', 'saved', 'failed', 'expired', 'detached']);
const outboxFields = { request_id: idSchema, workspace_instance_id: idSchema, conversation_id: idSchema, client_turn_id: idSchema, user_message_id: idSchema, assistant_message_id: idSchema, run_id: idSchema, payload_digest: z.string().regex(/^[0-9a-f]{64}$/), status: outboxStatusSchema, remote_run_id: idSchema.nullable(), response_digest: z.string().regex(/^[0-9a-f]{64}$/).nullable(), error_code: z.string().min(1).max(80).nullable(), last_ack_error: z.string().min(1).max(80).nullable().default(null), created_at: timestampSchema, updated_at: timestampSchema, last_checked_at: timestampSchema.nullable(), detached_reason: z.string().max(300).nullable() };
export const legacyOutboxTurnV1Schema = z.strictObject({ schema_version: z.literal(1), envelope: researchTurnEnvelopeV1Schema, ...outboxFields });
export const outboxTurnSchema = z.strictObject({ schema_version: z.literal(2), envelope: z.union([researchTurnEnvelopeV1Schema, researchTurnEnvelopeV2Schema]), ...outboxFields });
export const deletionReceiptSchema = z.strictObject({ id: idSchema, kind: z.enum(['conversation', 'personal_note']), target_id: idSchema, deleted_at: timestampSchema, privacy_epoch: z.number().int().positive() });
const workspaceFields = { instance_id: idSchema, portfolio_id: idSchema, root_generation: z.number().int().nonnegative(), migration: migrationStateSchema, journal: z.array(journalEntrySchema).max(10000), conversations: z.array(conversationSchema).max(1000), messages: z.array(messageSchema).max(20000), runs: z.array(analysisRunSchema).max(5000), sources: z.array(sourceSnapshotSchema).max(10000), policies: z.array(policySchema).max(1000), privacy_epoch: z.number().int().nonnegative().default(0), deletions: z.array(deletionReceiptSchema).max(5000).default([]) };
export const workspaceStateV2Schema = z.strictObject({ version: z.literal(2), ...workspaceFields, outbox: z.array(legacyOutboxTurnV1Schema).max(2000) });
export const workspaceStateSchema = z.strictObject({ version: z.literal(3), ...workspaceFields, outbox: z.array(outboxTurnSchema).max(2000) });
...
export function shanghaiDayKey(timestamp: string) { return new Date(Date.parse(timestamp) + 8 * 60 * 60 * 1000).toISOString().slice(0, 10); }
