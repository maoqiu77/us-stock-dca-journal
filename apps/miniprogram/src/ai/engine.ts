...
type FinancialPort = { snapshot(): Snapshot; overview(cutoff?: { throughDate: string; knownAt: string }, currency?: string): any; records(): any[]; positionDetail(value: string): any };
export function createAiEngine(workspace: WorkspacePort, financial: FinancialPort, runtime: Runtime, transport?: AiTransport, fakeProvider?: FakeAiProvider) {
  ...
  function buildContext(input: PreviewInput, conversationId?: string) {
    ...
    const portfolioPositions = overview.currencies?.length ? overview.currencies.flatMap((currency: string) => financial.overview(undefined, currency).positions) : overview.positions;
  ...
  function preparedFromTurn(turn: OutboxTurn) { const state = workspace.read(); return { conversation: workspace.conversation(turn.conversation_id).conversation, envelope: turn.envelope, turn, preview: { omissions: [] }, privacyEpoch: state.privacy_epoch } as unknown as ReturnType<typeof prepare>; }
