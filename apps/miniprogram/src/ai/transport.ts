...
export const aiCapabilitiesSchema = z.strictObject({ schemaVersion: z.literal(1), enabled: z.boolean(), authorized: z.boolean(), enrolled: z.boolean(), consented: z.boolean(), accessMode: z.enum(['closed_beta', 'public']), principalHash: z.string().regex(/^[a-f0-9]{64}$/), providerConfigured: z.boolean(), credentialMode: z.literal('sponsored'), byokEnabled: z.boolean(), consentVersion: z.number().int().positive(), usage: z.strictObject({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), used: z.number().int().nonnegative(), inflight: z.number().int().nonnegative(), timezone: z.literal('UTC') }), limits: z.strictObject({ dailyRequests: z.number().int().positive(), globalDailyRequests: z.number().int().positive(), maxInflight: z.number().int().positive(), maxInputBytes: z.number().int().positive(), maxOutputTokens: z.number().int().positive() }) });
