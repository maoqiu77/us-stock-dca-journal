  ...
  return async function handle(event: any, context: TrustedContext) {
    ...
    if (action === 'analyze') {
      ...
      try {
        ...
        for (const series of [...(receipt?.research?.series ?? []), ...(receipt?.portfolioSeries ?? [])]) {
          ...
          externalSources.push({ id:deps.id(),origin_entity_id:receipt!.id,origin_revision:receipt!.id,type:'quote',as_of:series.bars.slice(offset,offset+60).at(-1)?.time??series.fetchedAt,available_at:series.fetchedAt,content_digest:sha256(content),content });
          }
        }
