...
type Deps = { config: MarketConfig; access: MarketAccessPort; provider?: MarketProvider; receipts?: MarketReceiptStore; now(): string; id?(): string; userRate?: { budget: MarketBudgetPort; perMinute: number } };
...
export function createPortfolioMarketHandler(deps: Deps) {
  ...
  return async function handle(event: any, context: MarketTrustedContext) {
    ...
    if (!['search', 'quotes', 'bars', 'prepareAnalysisSnapshot'].includes(action)) return fail('UNKNOWN_ACTION', '不支持的行情 action。');
