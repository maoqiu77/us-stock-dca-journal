...
await build({ entryPoints: [join(root, 'src/market/index.ts')], outfile: join(out, 'market.cjs'), bundle: true, platform: 'node', format: 'cjs', target: 'node18', minify: true, legalComments: 'none' });
...
console.log(`Cloud bundles built: ${join(out, 'gateway.cjs')}, ${join(out, 'market.cjs')}`);
