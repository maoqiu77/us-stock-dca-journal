...
function fixture(output: unknown = { rows: [{ name: 'Invesco QQQ', code: 'QQQ', quantityText: '20', unitCostText: '10.5', costBasis: 'average_cost', currency: 'USD', accountLabel: null }], truncated: false }, removeFails = false) {
  let providerCalls = 0, removeCalls = 0, ids = 0;
  const objects = new Map<string, Uint8Array>();
  const store = createMemoryVisionTaskStore();
  const provider: HoldingVisionProvider = { configured: () => true, async recognize() { providerCalls++; return output; } };
  const objectStore = {
  ...
  const handler = createHoldingVisionHandler({
    config: { expectedAppId: 'wx-test', enabled: true, maxBytes: 4 * 1024 * 1024, maxPixels: 20_000_000, maxRows: 20, timeoutMs: 30_000, taskTtlMs: 60_000, dailyLimit: 3, maxInflight: 1 },
