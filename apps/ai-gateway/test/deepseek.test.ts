...
test('DeepSeek adapter uses Chat Completions fields and parses choices message JSON', async () => {
  let seen: any; const output = { schema_version: 99, request_id: 'spoofed', classification: 'user_original', mode: 'candidate_screen', summary: '这几只持仓目前走势不一，先看看各自的风险，再决定是否调整。', stance: 'observe', evidence: [], counterarguments: [], conditions: [], missing_information: [], candidates: [], next_questions: [] };
  ...
  assert.equal(seen.url, 'https://api.deepseek.com/chat/completions'); assert.equal(seen.body.max_tokens, 1500); assert.equal(seen.body.max_output_tokens, undefined); assert.equal(seen.body.stream, false); assert.equal(seen.init.headers.authorization, 'Bearer server-only'); assert.match(seen.body.messages[0].content, /All array fields are required/); assert.match(seen.body.messages[0].content, /portfolio_review and instrument_research/); assert.match(seen.body.messages[0].content, /恭喜🎉.*personal records confirm a gain/); assert.match(seen.body.messages[0].content, /Say “今天” only when/); assert.equal(result.providerId, 'deepseek'); assert.equal(result.credentialMode, 'sponsored');
  assert.equal(result.result.summary, output.summary);
