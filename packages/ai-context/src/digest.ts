...
export function sha256(text: string): string {
  // Mini-program hosts need not expose TextEncoder. Match its UTF-8 handling,
  // including replacement characters for unpaired UTF-16 surrogates.
  const bytes: number[] = [];
  ...
  view.setUint32(paddedLength - 4, length >>> 0);
  view.setUint32(paddedLength - 8, Math.floor(length / 0x100000000));
  const constants = [
...
export function payloadDigest(value: unknown) { return sha256(canonicalJson(value)); }
